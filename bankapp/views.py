...
def customer_register(request):
    ...
    Customer_Profile(customer_id = user.id, customer_name=customer_name, 
    account_number=account_number,father_name=father_name,email=email,
    mobile=mobile,dob=dob,balance=balance).save()
...
def customer(request):
    ...
    if request.method != 'POST':
        return render(request,'customer.html')
    try:
        user = auth.authenticate(username=request.POST.get('email'),password=request.POST.get('psw'))
        if user is not None:
...
def customer_profile(request):
    try:
        ...
        benificiary = Benificiary.objects.all().filter(customer_account = customer.account_number)
...
def add_benificiary(request):
    ...
    if Customer_Profile.objects.filter(account_number=account_number).exists():
        Benificiary(customer_account=customer_account, name = holder_name, nick_name = holder_nickname,
